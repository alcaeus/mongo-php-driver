...
#include "Writer_arginfo.h"

zend_class_entry* php_phongo_writer_ce;

static bool phongo_writer_fetch_key(php_phongo_writer_t* writer, char** key, size_t* key_len)
{
	if (!writer->key) {
...
static void phongo_writer_clear_key(php_phongo_writer_t* writer)
{
	if (!writer->key) {
		return;
	}
...
static PHP_METHOD(MongoDB_BSON_Writer, write)
{
	php_phongo_writer_t*   intern;
	php_phongo_document_t* document_intern;
	zval                   document;

	PHONGO_PARSE_PARAMETERS_NONE();

	intern = Z_WRITER_OBJ_P(getThis());
...
static PHP_METHOD(MongoDB_BSON_Writer, writeKey)
{
	char*                key;
	size_t               key_len;
	php_phongo_writer_t* intern;
	...
	phongo_writer_clear_key(intern);
...
static PHP_METHOD(MongoDB_BSON_Writer, writeBoolean)
{
	zend_bool            value;
	char*                key;
	size_t               key_len;
	...
	if (!phongo_writer_fetch_key(intern, &key, &key_len)) {
...
static PHP_METHOD(MongoDB_BSON_Writer, writeDouble)
{
	double               value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeInt)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeNull)
{
	char*                key;
	size_t               key_len;
	php_phongo_writer_t* intern;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeString)
{
	char*                value;
	size_t               value_len;
	char*                key;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeBinary)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeDecimal128)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeInt32)
{
	zend_long            value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeInt64)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeJavascript)
{
	zval*                    value;
	char*                    key;
	size_t                   key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeMaxKey)
{
	char*                key;
	size_t               key_len;
	php_phongo_writer_t* intern;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeMinKey)
{
	char*                key;
	size_t               key_len;
	php_phongo_writer_t* intern;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeObjectId)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeRegularExpression)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeTimestamp)
{
	zval*                   value;
	char*                   key;
	size_t                  key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeUTCDateTime)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeArray)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
static PHP_METHOD(MongoDB_BSON_Writer, writeDocument)
{
	zval*                value;
	char*                key;
	size_t               key_len;
...
/* MongoDB\BSON\Writer object handlers */
static zend_object_handlers php_phongo_handler_writer;

static void php_phongo_writer_free_object(zend_object* object)
{
	php_phongo_writer_t* intern = Z_OBJ_WRITER(object);
...
static zend_object* php_phongo_writer_create_object(zend_class_entry* class_type)
{
	php_phongo_writer_t* intern = zend_object_alloc(sizeof(php_phongo_writer_t), class_type);

	zend_object_std_init(&intern->std, class_type);
	object_properties_init(&intern->std, class_type);

	intern->std.handlers = &php_phongo_handler_writer;
	intern->bson         = bson_new();

	return &intern->std;
}

void php_phongo_writer_init_ce(INIT_FUNC_ARGS)
{
	php_phongo_writer_ce                = register_class_MongoDB_BSON_Writer();
	php_phongo_writer_ce->create_object = php_phongo_writer_create_object;

	memcpy(&php_phongo_handler_writer, phongo_get_std_object_handlers(), sizeof(zend_object_handlers));
	/* Re-assign default handler previously removed in php_phongo.c */
	php_phongo_handler_writer.clone_obj = zend_objects_clone_obj;
	php_phongo_handler_writer.free_obj  = php_phongo_writer_free_object;
	php_phongo_handler_writer.offset    = XtOffsetOf(php_phongo_writer_t, std);
}
